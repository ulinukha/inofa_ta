...
class _AddKemampuanState extends State<AddKemampuan> {
  ...
  Future<Null> _getListSkill() async {
    ...
    _listSkill.clear();
    final response = await http.get(BaseUrl.listSkill, headers: {
  ...
  tambahKemampuan() async {
    ...
    final response = await http.post(
        BaseUrl.getKemampuan + widget.userData.user.id_pengguna.toString(),
        headers: {
          'Authorization': 'Bearer ' + token,
        },
        body: {
  ...
  onSearch(String text) async {
    ...
    _listSkill.forEach((f) {
      if (f.kemampuan.contains(text)) _listSkillSearch.add(f);
    });
    setState(() {});
  }
                ...
                return Container(
                  child: GestureDetector(
                      onTap: () {
                        setState(() {
                          txtKemampuanId =
                              _listSkillSearch[i].id_kemampuan.toString();
                          tambahKemampuan();
                        });
                      },
                      child: Container(
                        ...
                        padding: EdgeInsets.only(
                            left: 24, right: 24, bottom: 5, top: 1),
                ...
                return Container(
                  child: GestureDetector(
                      onTap: () {
                        setState(() {
                          txtKemampuanId =
                              _listSkill[i].id_kemampuan.toString();
                          tambahKemampuan();
                        });
                      },
                      child: Container(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: <Widget>[
                              Row(children: <Widget>[Text(skills.kemampuan)])
                            ],
                          ),
                        ),
                      )),
