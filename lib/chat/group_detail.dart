...
class _DetailGroupState extends State<DetailGroup> {
  ...
  Future<UserModels> _getListAnggota()async{
    ...
    final response = await http.get(BaseUrl.listMember+widget.inovasis.id_inovasi.toString(),
    headers: {
  ...
  Future<Null> _getListRequestJoin()async{
    ...
    final response = await http.get(BaseUrl.listRequestJoin+widget.inovasis.id_inovasi.toString(),
    headers: {
      'Authorization': 'Bearer '+ token,
    });
    final data = jsonDecode(response.body);
  ...
  grandJoin()async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var token = prefs.getString('token');
    final response = await http.post(BaseUrl.grandJoinApi+penggunaId, 
    headers: {
      'Authorization': 'Bearer '+ token,
    },
    body: {
    ...
    return Scaffold(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: <Widget> [
              ...
              SizedBox(height: 15,),
              widget.userData.user.id_pengguna == widget.inovasis.pengguna_id?
              Container(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Center(
                      child: Container(
                        ...
                        child: RaisedButton(
                          ...
                          onPressed: () {
                            Navigator.push(
                              context, MaterialPageRoute(builder: (context)=>AddKemampuanInovasi(inovasis: widget.inovasis),
                              ),
                            );
                          },
      ...
      bottomNavigationBar: Container(
        ...
        child: Container(
          ...
          child: widget.userData.user.id_pengguna == widget.inovasis.pengguna_id?
          Container(
            child: RaisedButton(
              shape: new RoundedRectangleBorder(
                borderRadius: new BorderRadius.circular(8.0),
  ...
  Widget _requestList(){
    return _listRequest.length == 0?
    Center(
      child: Text('Tidak ada permintaan bergabung', style: TextStyle(fontSize: 14, color: Colors.black54))
    ) :
    Container(
        ...
        itemBuilder: (context, i){
          return Container(
            child: GestureDetector(
              onTap: () {
               Navigator.of(context).push(MaterialPageRoute(builder: (context) => RequstJoin(dataReq: _listRequest[i])));
              },
              child: Container(
                width: 400,
                color: Colors.transparent,
                            ...
                            Material(
                              ...
                              Container(
                                ...
                                child: Column(
                                  ...
                                  children: <Widget> [
                                    Container(
                                      child: new Text(_listRequest[i].display_name,
                              ...
                              //     ),
                              //     child: Image.asset('images/tolak.png', width: 35,),
                              //   )
                              // ),
                              // SizedBox(width: 20,),
                              // InkWell(
                              //   onTap: (){
                              //     setState(() {
                              //       penggunaId = _listRequest[i].pengguna_id.toString();
                              //       grandJoin();
                              //     });
                              //   },
                              //   child: Container(
                              ...
                              //       borderRadius: new BorderRadius.all(Radius.circular(50))
                              //     ),
                              //     child: Image.asset('images/terima.png', width: 35,),
                              //   )
                              // )
                          ]
                        ),
            ...
            children: <Widget>[
              Row(
                children: <Widget>[
                  ...
                  Container(
                    width: 250,
                    child: Text(widget.inovasis.display_name, style: TextStyle(
                      fontSize: 16
                    ),),
                  )
                ],
                  ...
                  children: <Widget>[
                    Row(
                      children: <Widget>[
                        ...
                        Container(
                          width: 250,
                          child: Text(_listAnggota[i].display_name, style: TextStyle(
                            fontSize: 16
                          ),),
                        )
                      ],
