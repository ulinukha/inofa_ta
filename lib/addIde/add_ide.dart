...
class _AddIdeState extends State<AddIde> {
  ...
  Future<String> _getListKategori() async {
    ...
    var response =
        await http.get(Uri.encodeFull(BaseUrl.listKategori), headers: {
  ...
  submit() async {
    try {
      var stream =
          http.ByteStream(DelegatingStream.typed(_thumbnail.openRead()));
      var length = await _thumbnail.length();
      var url = Uri.parse(BaseUrl.createInovasi);
      var request = http.MultipartRequest("POST", url);
      request.headers.addAll(headers);
      request.fields['pengguna_id'] =
          widget.userData.user.id_pengguna.toString();
